...
def generate_words(temperature, top_k, output_sequence_length):
    ...
    if start_sequence_size > 1:
        previous = tf.reshape(tf.convert_to_tensor(context_tensor[:,:-1]), (1, start_sequence_size-1))
    else:
        previous = tf.reshape(tf.convert_to_tensor(context_tensor), (1, 1))
...
if __name__ == "__main__":
    ...
    generate_words(temperature, top_k, output_sequence_length)